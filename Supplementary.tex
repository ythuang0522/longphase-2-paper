...
\textbf{a}, Per-position counts of alignments starting (front-clipped, above the axis) and ending (back-clipped, below) are scanned by a finite-state detector that opens a candidate interval at $\ge 5$ front-clips, extends it while starts exceed ends, and closes it when a comparable number of back-clips is observed (maximum 200~kb).
\textbf{b}, Inside an interval, each heterozygous site is scored by the mean number of mismatches to the phasing graph on ALT- versus REF-supporting reads; sites whose ALT reads carry $\ge 70\%$ of the mismatches are removed from all reads, because their alternate allele is a paralogue-specific difference rather than a heterozygous variant.}
\end{figure}
...
\textbf{a}, Per-read CpG states from the ML probability (methylated $\ge 0.8$, unmethylated $\le 0.2$, otherwise noise).
\textbf{b}, Reverse-strand calls are mapped to the forward CpG coordinate, strands are pooled, and a site is heterozygous when the minority state is at least 60\% of the majority state and noise is at most 20\% of depth.
\textbf{c}, Candidate heterozygous CpGs are refined by co-segregation with SNVs on the same reads (strong anchors) or with already accepted CpGs (weak, iterative).
\textbf{d}, Output record with methylated and unmethylated read names used by \code{phase}.}
\end{figure}
