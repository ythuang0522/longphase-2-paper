...
\todo{Add a panel \textbf{c}: the distribution of $\mathrm{MR}$ for benchmark-confirmed heterozygous sites versus benchmark-absent calls inside detected intervals, with the 0.7 threshold marked. Without it the threshold is asserted rather than shown, and this is the panel a reviewer will ask for.}}
\end{figure}
...
\todo{Two real-data panels would replace assertion with evidence, and both exist in the group's presentation material: (i) per-site distributions of the \code{ML} probability at heterozygous versus homozygous CpGs, with the 0.8 and 0.2 lines marked, justifying the two thresholds; (ii) alignment views of one accepted and two rejected candidate sites, showing what the linkage criterion in \textbf{d} excludes. Regenerate both on the HG002 data used here.}}
\end{figure}
